...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            expect(page.locator("#logoutBtn")).to_have_text("Log Out")
            ...
            expect(page.get_by_role("heading", name="Admin operations console")).to_be_visible()
            expect(page.get_by_text("browser@example.com")).to_be_visible()
            expect(page.get_by_role("heading", name="Database Tables")).to_be_visible()
            expect(page.locator("#dbTableSelect")).to_contain_text("users")
            page.select_option("#dbTableSelect", "users")
            expect(page.locator("#dbTable")).to_contain_text("browser@example.com")
            page.select_option("#dbTableSelect", "email_outbox")
            expect(page.locator("#dbJson")).to_contain_text("email_verification")
            ...
            expect(page.get_by_text("Schedule paused.")).to_be_visible()
            page.click("#startBtn")
            expect(page.get_by_role("heading", name="Full SEO Audit")).to_be_visible()
            expect(page.locator("#summaryView").get_by_text("Share: http")).to_be_visible(timeout=12000)
            expect(page.locator("#summaryView").get_by_text("HTML: http")).to_be_visible()
            expect(page.locator("#summaryView").get_by_text("JSON: http")).to_be_visible()
            expect(page.locator("#summaryView").get_by_text("HTML and On-page Signals: Image alt text gaps", exact=True)).to_be_visible()
